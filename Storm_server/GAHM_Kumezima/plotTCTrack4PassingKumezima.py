...
import os, sys
sys.path.append(os.path.abspath('/home/tkdals/Python_Earthscience/MPI_ex/MPI_mixing_depth_temp/mixing_dt_mpi/'))
...
# wind_arr = tc_chaba.usa_wind.data[0]
# gen_index = mt.gen(wind_arr)
# lmi_index = lmi(wind_arr)
# selected_indices = np.arange(gen_index, lmi_index+1)
# gen2lmi_chaba = tc_chaba.isel(date_time=selected_indices, drop=True)
#%%
...
# 태풍 진로 표시 (얇은 검은색 선)
ax.plot(valid_lon, valid_lat, color='black', linewidth=1)
...
# 날짜 레이블 추가
ax.scatter(filtered_lon, filtered_lat, color='red', zorder=5)
for lon, lat, date in zip(filtered_lon, filtered_lat, filtered_dates):
    ax.text(lon-0.3, lat+0.5, date.split(' ')[0], fontsize=10, ha='right')
